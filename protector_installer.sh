...
NEXTCLOUD_LOGIN=$(whiptail --inputbox "Enter your Nexcloud login:" 8 78 --title "Nexcloud Login" 3>&1 1>&2 2>&3)
NEXTCLOUD_PASSWORD=$(whiptail --passwordbox "Enter your Nexcloud password:" 8 78 --title "Nextcloud Password" 3>&1 1>&2 2>&3)
NEXTCLOUD_DB_PASSWORD=$(whiptail --passwordbox "Enter your Nexcloud Database password:" 8 78 --title "Nextcloud Database Password" 3>&1 1>&2 2>&3)
OINKCODE=$(whiptail --passwordbox "Enter your Oinkcode for Snort. If you don't have Snort account, please register at https://www.snort.org/users/sign_in in order to get newest Snort ules:" 8 78 --title "Snort Oinkcode" 3>&1 1>&2 2>&3)
...
if [ $(echo "$RAM_USAGE > $THRESHOLD" | bc) -eq 1 ]
then
    # Send an email alert
